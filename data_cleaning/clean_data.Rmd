...
data_delete <- data %>% 
  select(-c(INFANT.DEATHS, UNDER.FIVE.DEATHS, HIV.AIDS))
...
```{r}
# Danh sách các cột cần duyệt (bỏ cột LIFE.EXPECTANCY và BMI)
...
for (col in cols_to_loop) {
  ...
  # Thay thế outlier bằng giá trị trung vị
  values[is_outlier] <- median(values[!is_outlier], na.rm = TRUE)
  
  # Gán lại vào data_cleaned
